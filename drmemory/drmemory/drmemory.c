...
drmem_options_init(const char *opstr)
{
    ...
    /* WDFuzzer: init params */
    strcat(options.shm_name, options.fuzzer_id);
    strcat(options.pipe_name, options.fuzzer_id);
    char* m = strtok(options.coverage_module, ",");
    while (m) {
        modules[module_num] = m;
        module_num += 1;
        m = strtok(NULL, ",");
    }
...
event_exit(void)
{
    ...
    /* WDFuzzer: tell fuzzer side that target exits normally */
    if (!options.single) {
        WriteCommandToPipe('E');
        NOTIFY("Exit normally\n");
    }
...
{
    HANDLE f;
    f = CreateFile(pidfile, GENERIC_WRITE, FILE_SHARE_READ,
                          NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (f == INVALID_HANDLE_VALUE) {
        NOTIFY("cannot open %s", pidfile);
    } else {
        char pidbuf[16];
        BOOL ok;
        DWORD written;
...
event_module_load(void *drcontext, const module_data_t *info, bool loaded)
{
    ...
    if(!options.single)
        for(int i = 0;i < module_num;i++){
            if(strcmp(dr_module_preferred_name(info),modules[i]) == 0){
                module_entry_t* e = malloc(sizeof(module_entry_t));
                e->start = info->start;
                e->end = info->end;
                drvector_lock(&table->vector);
                drvector_append(&table->vector, e);
                drvector_unlock(&table->vector);
                e = drvector_get_entry(&table->vector, table->vector.entries - 1);
                NOTIFY("target load:%x,%x\n",e->start,e->end);
            }
        }
...
# endif
    ...
    char pidfile[256];
    dr_snprintf(pidfile, 256,"childpid_%s.txt", options.fuzzer_id);
    write_pid_to_file(pidfile,dr_get_process_id());
    ...
    /* WDFuzzer: init fuzzer side supports */
    if (!options.single) {
        setup_shmem();
        setup_pipe();
        setup_modules();
        if (strcmp(options.coverage_mode,"edge") == 0)
